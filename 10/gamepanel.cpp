...
void GamePanel::gameStatusPrecess(GameControl::GameStatus status)
{
    ...
    case GameControl::CallingLord:{
        ...
        for(int i=0;i<last3Card.size();++i)
        {
            ...
            m_last3Card[i]->setImage(m_cardBackImg,front);
            m_last3Card[i]->hide();
        }
        // 开始叫地主
    ...
    case GameControl::PlayingHand:
        ...
        for(int i=0;i<m_playerList.size();i++)
        {
            // 通过引用或者指针取数据，在这个数据对应的内存上修改，如果不是引用，对玩家的状态是没有影响的
...
void GamePanel::onDispatchCard()
{
    ...
    if(curMovePos>=100)
    {
        ...
        curPlayer->storeDispatchCard(card);     // storeDispatchCard发射信号，当前窗口就调用disposeCard
        Cards cs(card);
...
void GamePanel::onUserPlayHand()
{
    ...
    // 判断当前玩家的牌能不能压住上一家的牌
    if(m_gameCtl->getUserPlayer()!=m_gameCtl->getPendPlayer())
    {
...
QPixmap GamePanel::loadRoleImage(Player::Sex sex, Player::Direction direct, Player::Role role)
{
    ...
    // 加载图片 QPixmap  QImage  所有头像是朝向右的  水平镜像只有QImage提供了
    QImage image;
    int random = QRandomGenerator::global()->bounded(2);
    if(sex==Player::Man && role == Player::Farmer)
    {
        image.load(farmerMan.at(random));
    }
    else if(sex==Player::Man && role == Player::Lord)
    {
...
void GamePanel::initCountDown()
{
    ...
    connect(m_countDown,&CountDown::Cardtimeout,this,&GamePanel::onUserPass);

    UserPlayer* userplayer = m_gameCtl->getUserPlayer();
    connect(userplayer,&UserPlayer::startCountDown,this,[=](){
        if(userplayer != m_gameCtl->getPendPlayer() && m_gameCtl->getPendPlayer()!=nullptr)
        {
            m_countDown->showCountDowm();
        }
    });
}
