...
{
    // 1、对扑克牌进行分类：1张的，2张的，3张的，4张的u多少种
    classify(cards);
    // 2、对牌型进行分类
    judgeCardType();
}
...
bool PlayHand::canBeat(const PlayHand &other)
{
    ...
    // Hand_Single到Hand_Seq_Single类型全能管上
...
void PlayHand::classify(Cards &cards)
{
    ...
    int cardRecord[Card::Card_End];     // 记录点数对应的几张牌
...
void PlayHand::judgeCardType()
{
    ...
    else if(isSeqPair())
    {
        ...
        // 记录点数最小的牌
        m_pt = m_twoCard[0];            // 连对最小的值
        m_extra = m_twoCard.size();     // 连对的数量
    }
    else if(isSeqSingle())
    {
...
bool PlayHand::isSeqPair()
{
    if(m_oneCard.isEmpty()&&m_twoCard.size()>=3&&m_threeCard.isEmpty()&&m_fourCard.isEmpty())
    {
        ...
        if(m_twoCard[m_twoCard.size()-1]-m_twoCard[0]==m_twoCard.size()-1
            &&m_twoCard[m_twoCard.size()-1]<Card::Card_2)
        {
            return true;
        }
